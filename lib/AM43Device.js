...
class AM43Device extends EventEmitter {
  constructor(peripheral) {
    ...
    this.lightSensorConnected = false;
    this.lightLevel = 1;
  }
    ...
    this.blindsControlCharacteristic.on("data", (data) => {
      ...
      switch (data[1]) {
        case StaticVariables.AM43_COMMAND_ID_GET_POSITION:
          ...
          let hasLightSensor = dataArray[3] & 0x10
          if (hasLightSensor) {
            this.debugLog("light sensor detected")
            if (!this.lightSensorConnected) {
              this.lightSensorConnected = true
              this.emit('lightSensorConnected')
            }
          } else {
            this.debugLog("no light sensor detected")
            if (this.lightSensorConnected) {
              this.lightSensorConnected = false
              this.emit('lightSensorDisconnected')
            }
          }
  ...
  async sendCommandAsync(commandID, data) {
    ...
    this.debugLog(`Blinds characteristic available: ${this.blindsControlCharacteristic !== null && this.blindsControlCharacteristic !== undefined}`)
    try {
